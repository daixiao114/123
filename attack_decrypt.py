...
# 新增的UI元素
def add_known_pair(plaintext, ciphertext):
    pair = (plaintext, ciphertext)
    known_pairs.append(pair)
    update_known_pairs_listbox()


def update_known_pairs_listbox():
    listbox = root.children['!listbox']
    listbox.delete(0, tk.END)
    for pair in known_pairs:
        listbox.insert(tk.END, f"Plaintext: {pair[0]}, Ciphertext: {pair[1]}")


def generate_all_possible_keys(bit_length):
    return [f"{i:0{bit_length}b}" for i in range(2 ** bit_length)]


def start_meet_in_the_middle_attack():
    if len(known_pairs) == 0:
        messagebox.showerror("Error", "No known plaintext-ciphertext pairs provided.")
        return

    attack_results.clear()
    for K1 in generate_all_possible_keys(16):
        intermediate_values = {}
        for plaintext, _ in known_pairs:
            intermediate_value = Encrypt(ascii_to_binary(plaintext), K1)
            intermediate_values[intermediate_value] = K1

        for K2 in generate_all_possible_keys(16):
            for _, ciphertext in known_pairs:
                reverse_intermediate_value = Decrypt(ascii_to_binary(ciphertext), K2)
                if reverse_intermediate_value in intermediate_values:
                    K1_found = intermediate_values[reverse_intermediate_value]
                    attack_results.append((K1_found, K2))

    if attack_results:
        display_attack_results()
    else:
        messagebox.showinfo("Attack Result", "No keys found.")


def display_attack_results():
    results_window = tk.Toplevel(root)
    results_window.title("Attack Results")
    results_listbox = tk.Listbox(results_window, height=20, width=50)
    results_listbox.pack()
    for K1, K2 in attack_results:
        results_listbox.insert(tk.END, f"Found Keys: K1={K1}, K2={K2}")
...
# 中间相遇攻击相关的UI组件
tk.Label(root, text="Known Plaintext (ASCII):").pack()
known_plaintext_entry = tk.Entry(root)
known_plaintext_entry.pack()
tk.Label(root, text="Known Ciphertext (ASCII):").pack()
known_ciphertext_entry = tk.Entry(root)
known_ciphertext_entry.pack()
tk.Button(root, text="Add Known Pair",
          command=lambda: add_known_pair(known_plaintext_entry.get(), known_ciphertext_entry.get())).pack()
tk.Listbox(root, height=10, width=50).pack()
tk.Button(root, text="Start Attack", command=start_meet_in_the_middle_attack).pack()
