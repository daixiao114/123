...
def CBC_Encrypt(plain_text, key, IV):
    blocks = [plain_text[i:i + 2] for i in range(0, len(plain_text), 2)]
    ...
    for block in blocks:
        if len(block) < 2:
            ...
            continue
        binary_block = ascii_to_binary(block)
        xor_result = XOR(binary_block, previous_cipher_block)
        cipher_block = Encrypt(xor_result, key)
...
def CBC_Decrypt(cipher_text, key, IV):
    ...
    for cipher_block in cipher_blocks:
        decrypted_block = Decrypt(cipher_block, key)
        xor_result = XOR(decrypted_block, previous_cipher_block)
        plain_block = binary_to_ascii(xor_result)
...
# GUI部分
def validate_key_and_iv(input_str, iv_str):
    return all(c in '01' for c in input_str) and len(input_str) == 16 and all(c in '01' for c in iv_str) and len(
        iv_str) == 16


def show_CBC_encrypt_result():
    plaintext = plaintext_var.get()
    key = key_var.get()
    iv = iv_var.get()
    if not validate_key_and_iv(key, iv):
        messagebox.showerror("Input Error", "Key and IV must be 16-bit binary strings.")
        return
    ciphertext = CBC_Encrypt(plaintext, key, iv)
    ciphertext_var.set(ciphertext)


def show_CBC_decrypt_result():
...
# 创建主窗口
root = tk.Tk()
root.title("S-AES Encryption/Decryption Tool with CBC Mode and ASCII Support")

# 设置变量
plaintext_var = tk.StringVar()
key_var = tk.StringVar()
iv_var = tk.StringVar(value=generate_IV())  # 自动生成初始向量
ciphertext_var = tk.StringVar()

# GUI组件
tk.Label(root, text="Plaintext (ASCII):").pack()
tk.Entry(root, textvariable=plaintext_var).pack()
tk.Label(root, text="Key (16 bits):").pack()
tk.Entry(root, textvariable=key_var).pack()
tk.Label(root, text="IV (16 bits):").pack()
tk.Entry(root, textvariable=iv_var).pack()
tk.Button(root, text="Encrypt (CBC)", command=show_CBC_encrypt_result).pack()
tk.Label(root, text="Ciphertext (ASCII):").pack()
tk.Entry(root, textvariable=ciphertext_var, state='readonly').pack()
tk.Button(root, text="Decrypt (CBC)", command=show_CBC_decrypt_result).pack()
