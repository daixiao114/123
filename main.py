...
def show_encrypt_result():
    ...
    key = key_var.get()
    if not (validate_input(plaintext) and validate_input(key)):
        messagebox.showerror("Input Error", "Plaintext and Key must be 16-bit binary strings.")
        return
...
def show_decrypt_result():
    ...
    key = key_var.get()
    if not (validate_input(ciphertext) and validate_input(key)):
        messagebox.showerror("Input Error", "Ciphertext and Key must be 16-bit binary strings.")
        return
...
# 创建主窗口
root = tk.Tk()
root.title("S-AES Encryption/Decryption Tool")
...
# GUI组件
tk.Label(root, text="Plaintext (16 bits):").pack()
...
tk.Label(root, text="Ciphertext (16 bits):").pack()
