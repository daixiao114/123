...
def TripleEncrypt(plain_text, key):
    key1 = key[:16]
    key2 = key[16:32]
    key3 = key[32:]
    cipher_text = Encrypt(plain_text, key1)
    cipher_text = Encrypt(cipher_text, key2)
    cipher_text = Encrypt(cipher_text, key3)
    return cipher_text


def TripleDecrypt(cipher_text, key):
    key3 = key[32:]
    key2 = key[16:32]
    key1 = key[:16]
    plain_text = Decrypt(cipher_text, key3)
    plain_text = Decrypt(plain_text, key2)
    plain_text = Decrypt(plain_text, key1)
    return plain_text


def ascii_triple_encrypt(plain_text, key):
    encrypted_text = ''
    for i in range(0, len(plain_text), 2):
        block = plain_text[i:i + 2]
        if len(block) < 2:
            ...
            break
        binary_block = ascii_to_binary(block)
        encrypted_block = TripleEncrypt(binary_block, key)
        encrypted_chars = binary_to_ascii(encrypted_block)
        encrypted_text += encrypted_chars
    return encrypted_text


def ascii_triple_decrypt(cipher_text, key):
    decrypted_text = ''
    for i in range(0, len(cipher_text), 2):
        block = cipher_text[i:i + 2]
        if len(block) < 2:
            ...
            break
        binary_block = ascii_to_binary(block)
        decrypted_block = TripleDecrypt(binary_block, key)
        decrypted_chars = binary_to_ascii(decrypted_block)
        decrypted_text += decrypted_chars
    return decrypted_text
...
# 更新GUI部分
def validate_key(input_str, triple_encryption=False):
    if triple_encryption:
        return all(c in '01' for c in input_str) and len(input_str) == 48
    else:
...
def show_decrypt_result():
    ...
    if triple_encryption_var.get() == 1:
        if not validate_key(key, True):
            messagebox.showerror("Input Error", "Key must be a 32-bit binary string for double encryption.")
            return
        plaintext = ascii_triple_decrypt(ciphertext, key)
    else:
        if not validate_key(key, False):
            messagebox.showerror("Input Error", "Key must be a 16-bit binary string for single encryption.")
            return
...
triple_encryption_var = tk.IntVar()

# GUI组件
tk.Label(root, text="Plaintext (ASCII):").pack()
tk.Entry(root, textvariable=plaintext_var).pack()
tk.Label(root, text="Key (16 bits for single, 48 bits for triple encryption):").pack()
tk.Entry(root, textvariable=key_var).pack()
tk.Checkbutton(root, text="Use Triple Encryption", variable=triple_encryption_var).pack()
